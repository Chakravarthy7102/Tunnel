...
export default function ClientPage() {
	return <OrganizationIntegrationsCard />;
}

function OrganizationIntegrationsCard() {
				...
				>
					{shouldShowIntegration(organization.githubOrganization) && (
						<IntegrationRow
							...
							linkUrl={organization.githubOrganization ?
								`/${organization.slug}/settings/integrations/github` :
								getGithubAuthUrl({
					...
					<IntegrationRow
						...
						linkUrl={actorOrganizationMember.linkedGitlabAccount ?
							`/${organization.slug}/settings/integrations/gitlab` :
							getGitlabAuthUrl({
								organizationId: organization._id,
								organizationMemberId: actorOrganizationMember._id,
				...
				>
					{shouldShowIntegration(organization.linearOrganization) && (
						<IntegrationRow
							...
							linkUrl={organization.linearOrganization ?
								`/${organization.slug}/settings/integrations/linear` :
								getLinearAuthUrl({
					...
					{shouldShowIntegration(organization.jiraOrganization) && (
						<IntegrationRow
							...
							logo={<JiraIcon />}
							linkUrl={`/${organization.slug}/settings/integrations/jira`}
							buttonText={organization.jiraOrganization ? 'Manage' : 'Connect'}
							isPaid={true}
							isDisabled={false}
						/>
					)}
					{shouldShowIntegration(organization.asanaOrganization) && (
						<IntegrationRow
							...
							linkUrl={organization.asanaOrganization ?
								`/${organization.slug}/settings/integrations/asana` :
								getAsanaAuthUrl({
									isPersonalConnection: false,
									organizationMemberId: actorOrganizationMember._id,
							...
							buttonText={organization.asanaOrganization ? 'Manage' : 'Connect'}
							isPaid={true}
							isDisabled={false}
						/>
					)}
				...
				>
					<IntegrationRow
						...
						linkUrl={organization.slackOrganization ?
							`/${organization.slug}/settings/integrations/slack` :
							getSlackAuthUrl({
								isPersonalConnection: false,
								organizationMemberId: actorOrganizationMember._id,
