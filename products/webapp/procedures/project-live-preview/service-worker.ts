...
export const projectLivePreview_hasTunnelServiceWorker = defineProcedure({
	input: z.object({
		url: z.string(),
	}),
	async query({ input }) {
		try {
			const response = await ky.get(input.url + '/__tunnel-service-worker.js');
	...
	error: ({ error }) =>
		new ProcedureError("Couldn't get service worker", error),
});
