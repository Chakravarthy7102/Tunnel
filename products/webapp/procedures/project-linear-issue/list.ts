...
export const projectLinearIssue_listFiltersChoices = defineProcedure({
	...
	query: async ({ input }) => ($try(async function*() {
		switch (true) {
			case 'organization' in input: {
				...
				return ApiConvex.v.ProjectLinearIssue.listFiltersChoices({
					input: {
	...
	error: ({ error }) =>
		new ProcedureError("Couldn't list Linear filters", error),
});
