...
import { buildSveltePackage } from './build.ts';

export async function publishSveltePackage(options: {
	build?: boolean;
	release: Release;
}) {
	const bumpedPackageVersion = getBumpedPackageVersion({
		packageName: '@tunnel/svelte',
	});
	...
	const distDirpath = path.join(packageDirpaths.sveltePackage, '.build');
	...
	await octokit.createOrUpdateFiles({
		...
		changes: [
			{
				message: `@tunnel/svelte@${bumpedPackageVersion} [skip ci]`,
				files: {
					...(await (async () => {
	...
	await program
		...
		.action(
			async (options: {
				...
				await publishSveltePackage({
					build: options.build,
					release: options.release,
				});
				logger.info('Successfully published @tunnel/svelte');
				process.exit(0);
			},
		)
		.parseAsync();
