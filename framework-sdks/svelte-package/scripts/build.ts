...
export const buildSveltePackage = async (
	{ release, version }: { release: Release; version: string },
) =>
	build(
		packageDirpaths.sveltePackage,
		false,
		buildPreset({
...
if (esMain(import.meta)) {
	await program
		...
		.action(async (options: { release: Release | 'development' }) => {
			logger.info(`Building @tunnel/svelte (release: ${options.release})...`);
			await buildSveltePackage({
				release: options.release === 'development' ? null : options.release,
				version: tunnelPublicPackagesMetadata['@tunnel/svelte'].version,
			});
			logger.event(
				`Successfully built @tunnel/svelte (release: ${options.release})!`,
			);
		})
		.parseAsync();
}
