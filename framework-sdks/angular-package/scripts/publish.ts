...
import { buildAngularPackage } from './build.ts';

export async function publishAngularPackage(options: {
	build?: boolean;
	release: Release;
}) {
	const bumpedPackageVersion = getBumpedPackageVersion({
		packageName: '@tunnel/angular',
	});
	...
	const distDirpath = path.join(packageDirpaths.angularPackage, '.build');
	...
	await octokit.createOrUpdateFiles({
		...
		changes: [
			{
				message: `@tunnel/angular@${bumpedPackageVersion} [skip ci]`,
				files: {
					...(await (async () => {
	...
	await program
		...
		.action(
			async (options: {
				...
				await publishAngularPackage({
					build: options.build,
					release: options.release,
				});
				logger.info('Successfully published @tunnel/angular');
				process.exit(0);
			},
		)
		.parseAsync();
