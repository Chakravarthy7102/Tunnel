...
import { buildVuePackage } from './build.ts';

export async function publishVuePackage(options: {
	build?: boolean;
	release: Release;
}) {
	const bumpedPackageVersion = getBumpedPackageVersion({
		packageName: '@tunnel/vue',
	});
	...
	const distDirpath = path.join(packageDirpaths.vuePackage, '.build');
	...
	await octokit.createOrUpdateFiles({
		...
		changes: [
			{
				message: `@tunnel/vue@${bumpedPackageVersion} [skip ci]`,
				files: {
					...(await (async () => {
	...
	await program
		...
		.action(
			async (options: {
				...
				await publishVuePackage({
					build: options.build,
					release: options.release,
				});
				logger.info('Successfully published @tunnel/vue');
				process.exit(0);
			},
		)
		.parseAsync();
