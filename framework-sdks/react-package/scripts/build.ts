...
export const buildReactPackage = async (
	{ release, version }: { release: Release; version: string },
) =>
	build(
		packageDirpaths.reactPackage,
		false,
		buildPreset({
...
if (esMain(import.meta)) {
	await program
		...
		.action(async (options: { release: Release | 'development' }) => {
			logger.info(`Building @tunnel/react (release: ${options.release})...`);
			await buildReactPackage({
				release: options.release === 'development' ? null : options.release,
				version: tunnelPublicPackagesMetadata['@tunnel/react'].version,
			});
			logger.event(
				`Successfully built @tunnel/react (release: ${options.release})!`,
			);
		})
		.parseAsync();
}
