// @ts-nocheck
/* eslint-disable -- Copy-pasted from GitHub */
// dprint-ignore-file

/**
	@see https://stackoverflow.com/a/38254661/12581865
*/
function stringifyXpathExpression(expression: string) {
	...
	return `concat('${expression.replaceAll('\n', '').replaceAll(/'/g, `',"'",'`)}')`;
}
...
export class RobulaPlus {
   ...
   */
  public getRobustXPath(element: Element, document: Document): string | null {
   ...
   */
  public getElementByXPath(xPath: string, document: Document): HTMLElement | null {
		if (xPath === '') {
			return null;
		}
  ...
  /**
   * Returns, whether an xPath describes only the given element.
   *
   * @param xPath - A xPath string, describing the desired element.
   * @param element - The desired element.
   * @param document - The document to analyse, that contains the desired element.
  ...
  public transfAddId(xPath: XPath, element: Element): XPath[] {
    ...
    if (ancestor.id && !xPath.headHasAnyPredicates()) {
      ...
      newXPath.addPredicateToHead(`[@id=${stringifyXpathExpression(ancestor.id)}]`);
  ...
  public transfAddText(xPath: XPath, element: Element): XPath[] {
    ...
    if (ancestor.textContent && !xPath.headHasPositionPredicate() && !xPath.headHasTextPredicate()) {
      const newXPath: XPath = new XPath(xPath.getValue());
      newXPath.addPredicateToHead(`[contains(text(),${
				stringifyXpathExpression(ancestor.textContent)
			})]`);
      output.push(newXPath);
    }
    return output;
      ...
      for (const priorityAttribute of this.attributePriorizationList) {
        for (const attribute of ancestor.attributes) {
          if (attribute.name === priorityAttribute) {
            const newXPath: XPath = new XPath(xPath.getValue());
            newXPath.addPredicateToHead(`[@${attribute.name}=${
							stringifyXpathExpression(attribute.value)
						}]`);
            output.push(newXPath);
            break;
          }
  ...
  public transfAddAttributeSet(xPath: XPath, element: Element): XPath[] {
    ...
    if (!xPath.headHasAnyPredicates()) {
      ...
      for (const attributeSet of attributePowerSet) {
        let predicate: string = `[@${attributeSet[0].name}=${
					stringifyXpathExpression(attributeSet[0].value)
				}`;
        for (let i: number = 1; i < attributeSet.length; i++) {
          predicate += ` and @${attributeSet[i].name}=${
						stringifyXpathExpression(attributeSet[i].value)
					}`;
        }
        predicate += ']';
        const newXPath: XPath = new XPath(xPath.getValue());
