...
import { consoleWarn, isDataUrl, resolveUrl } from './utils.ts';
...
export async function replaceCssUrlToDataUrl(
	...
	for (const [rawUrl, url] of parseCssUrls(cssText, baseUrl)) {
		try {
			// eslint-disable-next-line no-await-in-loop -- Need to parse in order
...
function parseCssUrls(
	cssText: string,
	baseUrl: string | null,
): [string, string][] {
	const result: [string, string][] = [];

	cssText.replaceAll(URL_RE, (raw, quotation, url) => {
...
function toRE(url: string): RegExp {
	// eslint-disable-next-line no-useless-escape -- Need to escape
	const escaped = url.replaceAll(/([$()*+./?[\\\]^{|}])/g, '\\$1');
