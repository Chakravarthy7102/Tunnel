const _P = 'p'.codePointAt(0)!;
const _H = 'H'.codePointAt(0)!;
const _Y = 'Y'.codePointAt(0)!;
const _S = 's'.codePointAt(0)!;

let pngDataTable: ReturnType<typeof createPngDataTable> | undefined;
...
function searchStartOfPhys(uint8Array: Uint8Array) {
	...
	for (let i = length; i >= 4; i--) {
		if (
			uint8Array[i - 4] === 9 && uint8Array[i - 3] === _P &&
...
export function changePngDpi(
	...
	if (overwritepHYs) {
		...
		// if it was we could add a text chunk contaning some info, until desired
		// length is met.
...
// the pHYs signature block is preceded by the 4 bytes of length. The length of
// the block is always 9 bytes. So a phys block has always this signature:
// 0 0 0 9 p H Y s.
// However the data64 encoding aligns we will always find one of those 3 strings.
// this allow us to find this particular occurrence of the pHYs block without
