...
import { createCid, createIdPair } from '@-/database';
...
import { BadgesRow } from './thread-input/badges-row.tsx';
import { FilesRow } from './thread-input/files-row.tsx';
import { IntegrationDialog } from './thread-input/integration-dialog.tsx';
...
export function NewThreadInput({
	...
	const handleSubmit = () => {
		...
		const createCommentThreadAction = createDoc.action(
			...
			(create) =>
				create<typeof ProjectCommentThread_$commentsProviderData>({
					...
					xpathType: pageProps?.xpathType ?? 'similo',
		...
		addCommentThread.server((state) => {
			...
			state = addComment.action({
				comment: createCommentAction.flatDoc,
			})(state);
	...
	return (
		<div className={threadInputContainerVariants({ variant: uiProps.variant })}>
			<TiptapEditor
			...
			<div className="flex flex-col items-center justify-between w-full py-3 md:flex-row gap-2">
				...
				<div className="flex flex-row justify-end md:w-auto w-full items-center gap-x-2 md:justify-center ">
					<div className="flex">{projectSelectComponent}</div>
					...
					<div className="flex flex-row justify-center">
						<Button
							...
							className={cn(actorOrganizationMember ? 'rounded-r-none' : '')}
						>
							Create
						</Button>
